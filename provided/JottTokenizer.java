...
public class JottTokenizer {
    ...
    public TokenizerError handleCharacter(char c) {
        ...
        if (handlerMap == null) {
            // This should never happen because this mean should mean we've already hit an
            // issue and our state is faulted, but let's check just to ensure we don't have
            // a null ref
            return new TokenizerError(filename, lineNum, columnNum, state, tokenProgress, c);
        }
    ...
    public TokenizerError finalizeTokens() {
        if (state != TokenizerState.COMMENT) {
            ...
            if (!handler.tokenEnd()) {
                // Process current token if not already processed
                processToken();
            }
        }

        // Update the tokenizer state and return without error
    ...
    private void updateState(TokenizerState state) {
        ...
        if (state == TokenizerState.FAULTED) {
            // Clear the handler map if the state is faulted
            handlerMap = null;
            return;
        }

        // Grab the state's handler map from the constant(ish, thanks Java) collection
        // of handler maps
    ...
    private void processToken() {
        if (handler != null && handler.getCreatedTokenType() != null) {
            // Only process the token if the token's type is specified by the current
            // state's handler
            tokens.add(new Token(tokenProgress, filename, lineNum, handler.getCreatedTokenType()));
        }

        // Reset the token progress
        tokenProgress = "";
    }
    ...
    private static Map<TokenizerState, Map<Character, TokenizerStateHandler>> mapsInitializer() {
        ...
        TokenizerStateHandler relOpCompletionHandler = new TokenizerStateHandler(TokenizerState.START,
                TokenType.REL_OP);
